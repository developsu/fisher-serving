...
def paddingresize(obj_img, crop_size):
    ...
    # _kp2 = [[0, 0], [0, 0]]
    # xy = [[x1, x2], [y1, y2]]
    # for i, i_d in enumerate(_kp):
    #     for ii, ii_d in enumerate(_kp[i]):
    #         if _kp[i][ii]:
    #             _kp2[i][ii] = _kp[i][ii]
    #             _kp2[i][ii] = max(_kp2[i][ii], xy[ii][0])
    #             _kp2[i][ii] = min(_kp2[i][ii], xy[ii][1])
    #
    # _kp_r = [[None, None], [None, None]]
    # _kp_r[0][0] = _kp2[0][0] - x1 + 64
    # _kp_r[0][1] = _kp2[0][1] - y1 + 64
    # _kp_r[1][0] = _kp2[1][0] - x1 + 64
    # _kp_r[1][1] = _kp2[1][1] - y1 + 64
    ...
    if h >= w:
        ...
        # _kp_r[0][0] = int(_kp_r[0][0] * ratio)
        # _kp_r[0][1] = int(_kp_r[0][1] * ratio) + pad_l
        # _kp_r[1][0] = int(_kp_r[1][0] * ratio)
        # _kp_r[1][1] = int(_kp_r[1][1] * ratio) + pad_l

    return obj_img # _kp_r
...
async def get_detection_and_diseases(file: UploadFile) -> JSONResponse:
    ...
    for i, obj_img in enumerate(img_rois):
        img_rois[i] = paddingresize(obj_img, crop_size)
